...
> 🔴 **최우선 — 잘림 0 (NO-CROP) 규칙 (2026-06-19 원장 검수, ROMA 덱 계기)**: 위 "필요하면 crop"은 **가장자리에 의미가 없는 ambient 풀블리드 사진에만 적용하는 last-resort**다. **기본 원칙은 "슬롯 비율 = 생성 이미지 비율을 정확히 일치시켜 잘림 0"**이다. 절차: 레이아웃에서 슬롯의 정확한 비율(W:H)을 먼저 정하고 → **그 비율 그대로** `$imagegen` 프롬프트의 `aspect ratio strictly W:H`에 넣어 생성 → 비율이 일치하므로 `object-fit: cover`든 `contain`이든 **아무것도 잘리지 않는다**.
> - **다이어그램류(흐름·분기·비교·프로세스·단계·아이콘 행 = Mechanism/Process/Comparison)는 cover-crop 절대 금지.** 화살표·분기·끝 객체가 가장자리에서 잘리면 정보가 사라진다. 이 경우 반드시 **슬롯 비율을 이미지 비율과 일치 + `object-fit: contain`**으로 전체를 보존한다.
> - 슬롯이 이미 만들어졌는데 생성 이미지가 안 맞으면, **이미지를 crop하지 말고 (a) 슬롯 비율에 맞춰 재생성하거나 (b) 슬롯의 aspect-ratio를 이미지에 맞게 조정**한다. "레이아웃을 이미지에 끌려가게 바꾸지 않는다"는 *구도/품질*에 관한 것이지, *비율을 안 맞춰 잘라도 된다*는 뜻이 아니다 — 비율은 처음부터 일치시킨다.
> - `_validate_layout`/preview 육안 검수에서 **이미지 가장자리 콘텐츠(화살표·분기·객체 끝)가 잘리면 FAIL**로 보고 재생성/슬롯조정한다. "조금 잘림"도 통과 아님.
