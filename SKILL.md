...
2. **인터미션 — 인포그래픽 슬롯 설계 + `$imagegen` 생성** (Step 3.5): Codex 가 슬라이드/섹션을 훑어 어디에 어떤 인포그래픽이 들어가면 이해도가 올라갈지 판단하고, HTML 안에 실제 슬롯을 만든 뒤, 슬롯 크기에 맞춘 영문 프롬프트로 `$imagegen`을 직접 호출한다.
...
**이미지 수량 기본값**:
- **handouts / lab-reports**: 1~2개 이미지 슬롯을 만든다. 검사결과지는 PII 없는 해부·기전·검사 원리·안전 확인 이미지로 제한한다.
- **decks**: 최소 4개 이상의 이미지를 사용한다. 12장 덱에서는 핵심 정의·기전·절차·행동/비교 슬라이드에 우선 배치한다.
- 생략은 사용자가 명시적으로 텍스트-only를 요청하거나, PII/의학적 안전상 이미지가 부적절할 때만 가능하며 이유를 보고한다.
...
### Step 3.5 — 인포그래픽 슬롯 설계 + `$imagegen` 생성
...
**수량 기준**:
- **handouts / lab-reports**: 1~2개 이미지를 만든다. handout은 기전·행동·비교·안전 확인, lab-report는 검사 원리·장기 모식도·결과 이해 보조처럼 PII가 없는 이미지를 사용한다.
- **decks**: 최소 4개 이상의 이미지를 만든다. 표지 장식보다 Definition, Mechanism, Process, Action, Comparison 슬라이드에 우선 배치한다.
...
**수량 가이드**: handouts / lab-reports 는 1~2개. decks 는 최소 4개 이상이며, 핵심 이해를 돕는 슬라이드에 분산 배치한다.

#### 3.5.a.1 — `$imagegen` 생성·저장 절차

1. HTML에 먼저 `.ai-visual` 슬롯을 만든다. 슬롯 폭/높이는 mm 또는 px로 고정한다.
...
3. `$imagegen` 기본 내장 모드로 이미지별 1회씩 생성한다. 서로 다른 이미지는 하나의 batch가 아니라 별도 프롬프트로 만든다.
4. 생성 결과를 확인한 뒤, 선택본을 `%USERPROFILE%\.codex\generated_images\...`에서 `shared/assets/generated/{topic-slug}-{purpose}-YYYYMMDD.png`로 복사한다. 원본은 삭제하지 않는다. deck split/strip 슬롯은 슬롯 비율로 crop/fit 저장해 좌우 여백이 남지 않게 한다.
5. HTML에 `<figure class="ai-visual">`로 삽입하고, 필요한 설명은 `.ai-visual__caption` 또는 `.ai-visual__pin`으로 얹는다. 슬롯을 꽉 채워야 하는 generated deck image에는 `.ai-visual--fill`을 함께 붙인다.
...
핸드아웃/검사 결과지 HTML은 기본적으로 **Codex가 이미지 슬롯을 만들고 `$imagegen`으로 직접 생성한 이미지 1~2개**를 배치한다. 슬라이드 덱은 최소 4개 이상의 이미지를 사용한다. 원장님이 별도 이미지를 제공한 경우에는 그 이미지를 우선 사용하지만, 제공 이미지가 없으면 Codex가 프롬프트를 설계해 생성한다.

표준 처리 순서:

1. HTML 레이아웃 안에 이미지 슬롯을 먼저 만든다 (`.ai-visual` / `.ai-visual--hero` / `.ai-visual--portrait` / `.ai-visual--compact`, 또는 자료별 custom fixed-size frame).
2. 슬롯 폭/높이를 실측해 영문 프롬프트를 작성하고 `$imagegen`으로 생성한다. deck split/strip 슬롯은 full-bleed 구도와 “no blank side gutters”를 명시한다. 이미지 내부 텍스트, PII, 브랜드명은 금지한다.
3. 선택한 PNG/WebP/JPEG를 `shared/assets/generated/{topic-slug}-{purpose}-YYYYMMDD.png`처럼 의미 있는 파일명으로 복사한다. deck split/strip 슬롯은 슬롯 비율로 crop/fit 저장하고 `.ai-visual--fill`로 배치한다. `$imagegen` 기본 생성 폴더의 원본은 삭제하지 않는다.
4. 이미지 안의 글자는 쓰지 않고, 라벨·캡션·설명은 HTML 텍스트로 배치한다.
5. `python3 -m shared._validate_layout <html_path>` 실행
6. 필요 시 `python3 build.py` 후 `output/{kind}/{slug}-preview.png`를 직접 확인
...
규칙: 이미지 프롬프트나 파일명에 환자명/차트번호/생년월일/원본 검사 PDF를 넣지 않는다. 이미지 안의 한글·영문 텍스트는 피하고, 모든 라벨은 HTML로 처리한다. 자세한 절차는 `reference/image-assets.md`.
