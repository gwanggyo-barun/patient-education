...
2. **가독성 향상**: 이미지가 텍스트를 대체하거나 압축해 이해를 돕는 경우에만 사용한다. 이미지를 넣느라 본문이 작아지거나 카드 간격이 무너지면 이미지를 버린다.
...
4. **프롬프트 추적**: 생성본 옆의 `.prompt.md`에는 slide 번호, slide title, source text summary, visual intent, unique subject, negative constraints 를 남긴다.
...
- **handouts / lab-reports**: 보통 0~2개. 검사결과지는 PII 없는 해부·기전·검사 원리·안전 확인 이미지로 제한한다.
- **decks**: 정의·기전·절차·행동/비교 슬라이드 중 그림이 없으면 이해가 떨어지는 곳에만 분산 배치한다. 최소 개수 할당은 없다.
- 생략은 실패가 아니다. 내용에 직접 맞는 visual intent 가 없거나 레이아웃이 나빠지면 이미지를 생략하고 텍스트·표·아이콘·여백으로 가독성을 높인다.
...
- **handouts / lab-reports**: 보통 0~2개. handout은 기전·행동·비교·안전 확인, lab-report는 검사 원리·장기 모식도·결과 이해 보조처럼 PII가 없는 이미지로 제한한다.
...
**선정 가이드**: handouts / lab-reports 는 0~2개, decks 는 이해를 돕는 만큼만 사용한다. 숫자보다 visual intent 가 중요하다. 각 후보는 슬라이드 제목·핵심 문장과 직접 연결되어야 하며, 같은 deck/series 안에서 같은 구도·같은 subject 를 반복하지 않는다.
