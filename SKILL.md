...
| `lab-reports` (`topic=health-checkup`) | 위 5인 + **checkup-completeness** = 6 (모듈 누락·follow-up·신호등 일관성 추가 점검) |
...
Stage A · D 에 **checkup-completeness specialist 1명 추가** (총 6인 병렬). 점검 영역:
1. 모듈 완전성 — 입력에 언급된 검사가 결과지에 모두 반영
2. follow-up 일정 — 각 이상·경계 소견에 next-step 명시
3. §0 신호등 ↔ 본문 일관성
4. §9 checklist 5 카테고리 균형 (즉시치료/생활습관/재검/전문과/경고)
5. 페이지 분할 균형

상세: `reference/quality-agents/checkup-completeness.md`.
