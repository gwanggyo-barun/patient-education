...
> **Backward compat**: 기존 lab-reports TARGETS가 `title: "[1063] 김종혁 — 골 대사 검사 (2026-04-29)"` 형식을 사용하면 `_notion_sync.py`가 자동으로 chart_no/patient_name/note를 파싱한다. 신규 항목은 explicit 필드 사용 권장.

### handouts → 📨 환자 유인물 DB 카테고리

`category` 값은 다음 6종 중 하나만 허용 (DB select option):
...
2. `git pull --rebase && git add . && git commit -m "Add lab report: 박순정 (17492)" && git push`
3. CI(GitHub Actions, ~80초 안에 완료):
   - Playwright PDF 빌드
   - GitHub Pages 배포 → `https://gwanggyo-barun.github.io/patient-education/lab-reports/general-checkup/박순정/`
   - **kind에 따라 자동 라우팅 → 해당 DB에 행 upsert**
4. 노션에서 환자 페이지 클릭 → 🌐 HTML / 📄 PDF 링크 한 번에 열림
