...
2. 슬롯 실측값으로 영문 프롬프트를 작성한다. PII, 제품명, 브랜드명, 이미지 내부 텍스트는 금지한다.
3. `$imagegen` 기본 내장 모드로 이미지별 1회씩 생성한다. 서로 다른 이미지는 하나의 batch가 아니라 별도 프롬프트로 만든다.
4. 생성 결과를 확인한 뒤, 선택본을 `%USERPROFILE%\.codex\generated_images\...`에서 `shared/assets/generated/{topic-slug}-{purpose}-YYYYMMDD.png`로 복사한다. 원본은 삭제하지 않는다.
5. HTML에 `<figure class="ai-visual">`로 삽입하고, 필요한 설명은 `.ai-visual__caption` 또는 `.ai-visual__pin`으로 얹는다.
...
2. 슬롯 폭/높이를 실측해 영문 프롬프트를 작성하고 `$imagegen`으로 생성한다. 이미지 내부 텍스트, PII, 브랜드명은 금지한다.
3. 선택한 PNG/WebP/JPEG를 `shared/assets/generated/{topic-slug}-{purpose}-YYYYMMDD.png`처럼 의미 있는 파일명으로 복사한다. `$imagegen` 기본 생성 폴더의 원본은 삭제하지 않는다.
