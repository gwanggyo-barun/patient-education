...
- ⭐ **"paint 겹침" 주의 (2026-06-06 사용자 진단 + 코덱스 감수)**: 기하학적 rect 는 안 겹쳐도 ① 이미지 figure 의 `box-shadow`(x-offset 0 + 큰 blur)가 좌측 거터로 번지거나 ② figure 의 연한 배경 박스(#f8fbff 등)가 이미지를 감싸 보이면 PDF(print)에서 "텍스트 박스와 겹쳐 보인다". 화면 캡쳐엔 잘 안 보이고 PDF 에서만 도드라짐. → 이미지 그림자는 약하게(blur≤9·opacity≤0.08, 좌측 번짐 최소), `.paper-visual`에 `overflow:hidden`+border-radius 로 배경박스·이미지를 같이 라운드 클립, 거터 ≥36px. **검증은 실제 PDF 픽셀로(DOM rect 로는 paint 겹침 못 잡음).**
