...
## Phase 1 — 인포그래픽 자동 제안 (decks / handouts, lab-reports 필요시)

### 언제 트리거되는가

`build.py` 와 `python3 -m shared._validate_layout` 가 둘 다 통과한 직후. 텍스트와 레이아웃이 일단 확정된 지점에서만 제안한다 — 그 전에 제안하면 슬롯 위치가 바뀔 수 있어 무의미. **레이아웃 확정 후에야 정확한 슬롯 폭/높이 측정 가능**.
