...
- **Phase 1 (슬롯 설계 + `$imagegen` 생성)**: 초안 HTML 이 빌드·검증 통과한 직후, Codex 가 슬라이드/섹션을 훑어 인포그래픽 후보를 식별하고, 실제 HTML 슬롯을 만든 뒤 슬롯 크기에 맞춘 영문 프롬프트로 `$imagegen`을 직접 호출한다.
- **Phase 2 (배치 & 최종화)**: 생성된 PNG/WebP/JPEG 선택본을 `shared/assets/generated/` 에 복사하고, 적절한 `.ai-visual` 변형으로 HTML 에 삽입한 다음 재빌드·검증·푸시한다.

판단 기준:
- **품질·가독성 우선**: 이미지 개수를 채우지 않는다. 삽입 후 텍스트가 작아지거나 정보 위계가 흐려지면 만들지 않는다.
- **handouts / lab-reports**: 보통 0~2개. lab-reports 는 환자 PII 없는 검사 원리·해부·결과 이해 보조 이미지로 제한한다.
...
4. `$imagegen` 기본 생성 폴더의 원본은 삭제하지 않고, 선택본만 `shared/assets/generated/`에 복사한다.
5. 파일명은 `{topic-slug}-{purpose}-YYYYMMDD.png`처럼 주제와 용도가 드러나게 짓는다.
6. 원본 프롬프트가 있으면 같은 이름의 `.prompt.md`로 남긴다. API/CLI로 생성한 경우에는 `.prompt.json`도 허용한다.
7. **이미지 생성은 반드시 slot-first**: HTML/CSS 슬롯을 먼저 만들고, 실제 슬롯 폭·높이와 비율을 잰 뒤 그 값으로 프롬프트를 작성한다. 이미지 파일을 먼저 만든 뒤 CSS로 억지로 맞추는 흐름은 금지한다.
8. 기존 handout 보강 작업에서는 로고/QR을 제외한 의미 있는 이미지가 하나라도 이미 있으면 스킵한다. 이미지가 없는 handout만 새 slot-first 이미지 대상으로 삼는다.
9. **내용 직접 연결**: prompt subject 는 해당 slide/section 의 제목·핵심 문장·표/카드 내용에서 온 구체적 개념이어야 한다. "clinic background", "medical icon strip", "organ-themed infographic"처럼 어디에나 붙일 수 있는 이미지는 실패다.
10. **중복 금지**: 같은 파일 재사용 금지. 파일명만 다른 동일 구도, 같은 generic strip, 같은 subject 반복도 중복으로 본다. deck/series 검수에서 `placements == unique_assets`, reused count 0 이 되어야 한다.
11. **불확실하면 생략**: 정확히 맞는 그림을 만들 수 없으면 이미지를 넣지 않는다. 이미지 생략은 실패가 아니지만 무관한 이미지는 실패다.

## Phase 1 — 인포그래픽 슬롯 설계 + `$imagegen` 생성
...
**선정 가이드**: handouts / lab-reports → 0~2개. 12장 deck → 이해를 크게 돕는 만큼만. 수량이 아니라 slide/section 별 visual intent 와 중복 없음이 기준이다.
...
5. `$imagegen` 기본 내장 모드로 후보별 1장씩 생성한다. 서로 다른 이미지를 하나의 프롬프트에 묶지 않는다.
6. 생성 이미지를 확인하고, 선택본을 `shared/assets/generated/`에 복사한다.
7. 같은 이름의 `.prompt.md`에 slide/section, source text summary, visual intent, unique subject, negative constraints 를 저장한다.
...
2. 같은 이름의 `.prompt.md` 에 원본 프롬프트와 함께 slide/section, source text summary, visual intent, unique subject, negative constraints 를 저장한다.
