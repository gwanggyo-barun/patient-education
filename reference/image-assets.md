...
- **Phase 1 (슬롯 설계 + 이미지/spec 생성)**: 초안 HTML 이 빌드·검증 통과한 직후, Codex 가 슬라이드/섹션을 훑어 인포그래픽 후보를 식별하고, 실제 HTML 슬롯을 만든 뒤 슬롯 크기에 맞춘 영문 프롬프트 또는 SVG spec 을 작성한다.
- **Phase 2 (배치 & 최종화)**: 생성된 PNG/WebP/JPEG/SVG 선택본을 `shared/assets/generated/` 에 저장하고, 적절한 `.ai-visual` 또는 handout 전용 figure 로 HTML 에 삽입한 다음 재빌드·검증·푸시한다.
...
4. 사진·질감·복잡한 해부 일러스트는 `$imagegen`으로 생성한다. 절차/장비/행동 요약처럼 정확한 정렬이 중요한 단순 도식은 코드 네이티브 SVG로 생성할 수 있다.
5. `$imagegen` 기본 생성 폴더의 원본은 삭제하지 않고, 선택본만 `shared/assets/generated/`에 복사한다.
6. 파일명은 `{topic-slug}-{purpose}-YYYYMMDD.{png|webp|jpg|svg}`처럼 주제와 용도가 드러나게 짓는다.
7. 원본 프롬프트나 SVG spec 은 같은 이름의 `.prompt.md`로 남긴다. API/CLI로 생성한 경우에는 `.prompt.json`도 허용한다.
8. **이미지 생성은 반드시 slot-first**: HTML/CSS 슬롯을 먼저 만들고, 실제 슬롯 폭·높이와 비율을 잰 뒤 그 값으로 프롬프트/spec 을 작성한다. 이미지 파일을 먼저 만든 뒤 CSS로 억지로 맞추는 흐름은 금지한다.
9. 기존 handout 보강 작업에서는 로고/QR을 제외한 의미 있는 이미지가 하나라도 이미 있으면 스킵한다. 이미지가 없는 handout만 새 slot-first 이미지 대상으로 삼는다.
10. **내용 직접 연결**: prompt/spec subject 는 해당 slide/section 의 제목·핵심 문장·표/카드 내용에서 온 구체적 개념이어야 한다. "clinic background", "medical icon strip", "organ-themed infographic"처럼 어디에나 붙일 수 있는 이미지는 실패다.
...
12. **중복 금지**: 같은 파일 재사용 금지. 파일명만 다른 동일 구도, 같은 generic strip, 같은 subject 반복도 중복으로 본다. deck/series 검수에서 `placements == unique_assets`, reused count 0 이 되어야 한다.
13. **불확실하면 생략**: 정확히 맞는 그림을 만들 수 없으면 이미지를 넣지 않는다. 이미지 생략은 실패가 아니지만 무관한 이미지는 실패다.

## Phase 1 — 인포그래픽 슬롯 설계 + 이미지/spec 생성
...
⚠️ **순서 금지 사항**: `$imagegen`을 먼저 호출하거나 SVG를 먼저 그린 뒤 그 결과물에 맞춰 레이아웃을 바꾸지 않는다. 먼저 HTML에 슬롯을 배치하고 `_validate_layout` 또는 Playwright screenshot으로 슬롯이 실제 자료 안에서 성립하는지 확인한 다음 이미지를 생성한다. 생성 결과가 슬롯과 어긋나면 이미지를 crop/resize하거나 재생성하고, 레이아웃을 이미지에 끌려가게 바꾸지 않는다.
...
**선정 가이드**: handouts / lab-reports → 대개 0~2개지만, 서로 다른 교육 포인트를 설명하는 전용 슬롯이면 여러 개도 가능하다. 12장 deck → 이해를 크게 돕는 만큼만. 수량이 아니라 slide/section 별 visual intent 와 중복 없음이 기준이다.

### 이미지/spec 생성·저장 절차

1. 내용 적합성 게이트를 통과한 후보만 남긴다.
2. 후보마다 실제 슬롯을 먼저 만든다. 슬롯은 fixed width/height 또는 aspect-ratio로 안정화한다.
3. 같은 deck/series 안의 기존 이미지 파일·프롬프트와 비교해 중복 subject/구도가 아닌지 확인한다.
4. 슬롯 실측값으로 영문 프롬프트를 작성한다. 텍스트·숫자·PII는 이미지에 넣지 않는다.
5. 복잡한 raster 일러스트는 `$imagegen` 기본 내장 모드로 후보별 1장씩 생성한다. 단순 절차/장비/행동 도식은 코드 네이티브 SVG로 생성할 수 있다. 서로 다른 이미지를 하나의 프롬프트/spec 에 묶지 않는다.
...
8. HTML 라벨·캡션·핀을 얹고, `_validate_layout` + preview PNG 육안 확인까지 끝낸다.
...
"horizontal 4-step medical icon row"}, 
...
- For N-column visuals, specify icon CENTER positions in % per §0.b
...
| 토큰 | 채우는 방법 |
|---|---|
| `{style descriptor}` | 콘텐츠 형식: `"clean medical banner illustration"`, `"horizontal 4-step icon row"`, `"side-by-side comparison diagram"` 등 |
...
`$imagegen` 결과 또는 코드 네이티브 SVG가 생성되면 다음을 자동 수행:

1. 선택한 PNG/WebP/JPEG/SVG 를 **제안 시 정한 파일명 그대로** `shared/assets/generated/{topic-slug}-{slot-key}.{ext}` 에 저장한다. `$imagegen` 기본 생성 폴더의 원본은 삭제하지 않는다.
2. 같은 이름의 `.prompt.md` 에 원본 프롬프트/spec 과 함께 slide/section, source text summary, visual intent, unique subject, slot size/ratio, negative constraints 를 저장한다.
...
## 기본 워크플로우: `$imagegen` 또는 코드 네이티브 SVG 직접 생성

1. HTML에 이미지 슬롯을 만든다.
2. 슬롯 실측값으로 영문 프롬프트 또는 SVG spec 을 작성한다.
3. Codex가 `$imagegen`을 호출하거나 코드 네이티브 SVG를 생성한다.
4. 가장 좋은 컷/spec 결과를 `shared/assets/generated/`에 저장하고, `.ai-visual` 또는 handout 전용 figure 컴포넌트로 배치한다.
