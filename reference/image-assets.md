...
수량 기준:
- **handouts / lab-reports**: 1~2개 이미지. lab-reports 는 환자 PII 없는 검사 원리·해부·결과 이해 보조 이미지로 제한한다.
- **decks**: 최소 4개 이상의 이미지. Definition, Mechanism, Process, Action, Comparison 슬라이드에 우선 배치한다.
...
**수량 가이드**: handouts / lab-reports → 1~2개. 12장 deck → 최소 4개 이상.

### `$imagegen` 생성·저장 절차

1. 후보마다 실제 슬롯을 먼저 만든다. 슬롯은 fixed width/height 또는 aspect-ratio로 안정화한다.
2. 슬롯 실측값으로 영문 프롬프트를 작성한다. 텍스트·숫자·PII는 이미지에 넣지 않는다.
3. `$imagegen` 기본 내장 모드로 후보별 1장씩 생성한다. 서로 다른 이미지를 하나의 프롬프트에 묶지 않는다.
4. 생성 이미지를 확인하고, 선택본을 `shared/assets/generated/`에 복사한다.
5. HTML 라벨·캡션·핀을 얹고, `_validate_layout` + preview PNG 육안 확인까지 끝낸다.
