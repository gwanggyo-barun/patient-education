...
def load_asset_manifest(*, refresh: bool = False) -> dict[str, Any]:
    """Return the asset manifest as a dict {key: entry}. Cached per process.

    Returns an empty dict if the manifest file is missing — callers should
    treat that as "no asset registry yet" (data-asset references will fail
    ...
    try:
        ...
        _MANIFEST_CACHE = {}
        return _MANIFEST_CACHE
    _MANIFEST_CACHE = data.get("assets", {}) or {}
    return _MANIFEST_CACHE
