...
- 약 1,000개 아이콘에 한국어 alt가 시드되어 있음 (`tools/healthicons_alt_ko.json`).
  나머지(typography·shapes·contraceptives·zoonoses·malaria-specific diagnostics 등 
  의원 도메인과 무관한 카테고리)는 use site에서 alt 직접 작성 권장 — 빈 alt 상태에서 
  빌드는 `data-asset='X' has no alt_ko in manifest` warning을 띄움.
- **filled → outline 자동 fallback**: 시드 파일에는 `hi-filled-X` 만 적으면 sync가 
  `hi-outline-X` 트윈에도 같은 한글 alt를 자동 적용. 같은 아이콘 글리프의 두 스타일이 
  같은 의미를 가리키므로 작가 노동력 절반으로 같은 커버리지 확보. 두 스타일에 
  다른 alt가 필요하면 outline 키도 따로 적으면 된다 (직접 매칭이 우선).
